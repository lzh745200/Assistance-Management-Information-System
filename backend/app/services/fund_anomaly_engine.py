...
def should_escalate_approval(risk_score, fund_amount):
    if risk_score >= RISK_ESCALATE_SCORE:
        return True
    if fund_amount >= RISK_ESCALATE_AMOUNT:
        return True
    return False
...
def should_block_status_transition(has_anomaly, risk_score, target_status):
    if not has_anomaly:
        return False
    if target_status.lower() in CRITICAL_STATUSES:
        return True
    return risk_score >= 25


def detect_fund_anomalies_full(
    db, fund_id, fund_name="", fund_amount=0.0, health_score=100.0, project_id=None
):
    anomalies = []

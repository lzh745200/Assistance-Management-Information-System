...
class TestAnalyzeData:
    ...
    @pytest.mark.asyncio
    async def test_analyze_trend_with_db(self, service, admin_user):
        """测试趋势分析"""
        mock_db = MagicMock()
        ...
        result = await service.analyze_data({}, analysis_type="trend", db=mock_db, user=admin_user)
    ...
    @pytest.mark.asyncio
    async def test_analyze_fund_efficiency_with_db(self, service, admin_user):
        """测试经费效率分析"""
        mock_db = MagicMock()
        # 村庄级聚合
        ...
        result = await service.analyze_data({}, analysis_type="fund_efficiency", db=mock_db, user=admin_user)
        assert result["type"] == "fund_efficiency"
        assert "global" in result
        assert "by_village" in result

    @pytest.mark.asyncio
    async def test_analyze_compare_with_db(self, service, admin_user):
        """测试村庄对比分析"""
        mock_db = MagicMock()
        mock_db.query.return_value.join.return_value.scalar.side_effect = [2023, 2023]
        ...
        result = await service.analyze_data({}, analysis_type="compare", db=mock_db, user=admin_user)
...
class TestAnalyzeIncomeTrend:
    ...
    def test_analyze_income_trend_success(self):
        ...
        mock_db = MagicMock()
        # VillageIncome 需 join SupportedVillage，mock 链包含 .join.return_value
        mock_db.query.return_value.join.return_value.group_by.return_value.order_by.return_value.all.return_value = [
            (2021, 10000.0, 5000.0, 5),
            (2022, 11000.0, 5500.0, 5),
            (2023, 12000.0, 6000.0, 5),
        ...
        result = service.analyze_income_trend(mock_db, user=admin_user)
        assert result["type"] == "income_trend"
        assert result["status"] == "completed"
...
class TestAnalyzeFundEfficiency:
    ...
    def test_analyze_fund_efficiency_success(self):
        ...
        result = service.analyze_fund_efficiency(mock_db, user=admin_user)
        assert result["type"] == "fund_efficiency"
        assert "global" in result
        assert "by_village" in result
...
class TestCompareVillages:
    ...
    def test_compare_villages_success(self):
        ...
        mock_db = MagicMock()
        # latest_year 子查询也经过 join + filter_by_data_scope，scalar 在 join.return_value 上
        mock_db.query.return_value.join.return_value.scalar.side_effect = [2023, 2023]
        mock_db.query.return_value.join.return_value.filter.return_value.group_by.return_value.all.side_effect = [
        ...
        result = service.compare_villages(mock_db, user=admin_user)
        assert result["type"] == "village_comparison"
    ...
    def test_compare_villages_no_income_data(self):
        ...
        mock_db = MagicMock()
        mock_db.query.return_value.join.return_value.scalar.side_effect = [None, 2023]
...
class TestForecastIncomeTrend:
    ...
    def test_forecast_income_trend_success(self):
        ...
        with patch('sklearn.linear_model.LinearRegression') as mock_lr:
            ...
            result = service.forecast_income_trend(mock_db, forecast_years=1, user=admin_user)
    ...
    def test_forecast_income_trend_insufficient_data(self):
        ...
        result = service.forecast_income_trend(mock_db, forecast_years=2, user=admin_user)
...
class TestForecastFundCompletion:
    ...
    def test_forecast_fund_completion_low_risk(self):
        ...
        result = service.forecast_fund_completion(mock_db, user=admin_user)

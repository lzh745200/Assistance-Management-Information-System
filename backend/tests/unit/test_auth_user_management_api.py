...
class TestBatchCreate:
    ...
    def test_batch_create_non_admin(self, client):
        _setup_regular_user(client)
        response = client.post(
            f"{self.prefix}/batch-create",
            json={"count": 2, "prefix": "user", "start_num": 1},
        )
    ...
    def test_batch_create_success(self, client):
        ...
        mock_db, mock_query = _make_mock_db()
        mock_query.filter.return_value.first.side_effect = [None, None, None]
        _override_db(client, mock_db)
        ...
        with patch("app.api.v1.auth.user_management.generate_password", return_value="genpwd1"):
            with patch("app.api.v1.auth.user_management.get_password_hash", return_value="hashed"):
                with patch("app.api.v1.auth.user_management.is_superuser", return_value=False):
                    response = client.post(
                        f"{self.prefix}/batch-create",
                        json={"count": 3, "prefix": "user", "start_num": 1, "role": "operator"},
                    )
                    assert response.status_code == 200
                    data = response.json()
                    assert data["success"] is True
                    assert data["data"]["accounts"][0]["username"] == "user001"
                    assert len(data["data"]["accounts"]) == 3

    def test_batch_create_skip_existing(self, client):
        admin = _setup_admin_user(client)
        call_count = [0]
        def first_side():
            call_count[0] += 1
            ...
            return None
        mock_db, mock_query = _make_mock_db()
        mock_query.filter.return_value.first.side_effect = first_side
        _override_db(client, mock_db)
    ...
    def test_batch_create_superadmin_role(self, client):
        admin = _setup_admin_user(client)
        mock_db, mock_query = _make_mock_db()
        ...
        with patch("app.api.v1.auth.user_management.generate_password", return_value="genpwd1"):
            with patch("app.api.v1.auth.user_management.get_password_hash", return_value="hashed"):
                with patch("app.api.v1.auth.user_management.is_superuser", return_value=True):
                    response = client.post(
                        f"{self.prefix}/batch-create",
                        json={"count": 1, "prefix": "super", "start_num": 1, "role": "super_admin"},
                    )
                    assert response.status_code == 200

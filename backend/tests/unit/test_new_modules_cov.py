...
Targets:
  ...
  - app/core/policy_enforcement.py
"""
...
from app.core.policy_enforcement import (
    check_module_read_policy,
    check_module_write_policy,
)
...
# ── 5. policy_enforcement.py ─────────────────────────────────────────


class TestCheckModuleWritePolicy:
    def _make_request(self, method="POST"):
        req = MagicMock(spec=Request)
        req.method = method
        return req

    def test_get_method_passthrough(self, mock_db, admin_user):
        """GET requests are not intercepted."""
        checker = check_module_write_policy("funds")
        req = self._make_request("GET")
        result = checker(request=req, current_user=admin_user, db=mock_db)
        assert result is None

    def test_head_method_passthrough(self, mock_db, regular_user):
        """HEAD requests are not intercepted."""
        checker = check_module_write_policy("funds")
        req = self._make_request("HEAD")
        result = checker(request=req, current_user=regular_user, db=mock_db)
        assert result is None

    def test_options_method_passthrough(self, mock_db, regular_user):
        """OPTIONS requests are not intercepted."""
        checker = check_module_write_policy("funds")
        req = self._make_request("OPTIONS")
        result = checker(request=req, current_user=regular_user, db=mock_db)
        assert result is None

    def test_superuser_bypass(self, mock_db):
        """super_admin bypasses policy."""
        user = MagicMock()
        ...
        result = checker(request=req, current_user=user, db=mock_db)
        assert result is None

    def test_is_superuser_attr_bypass(self, mock_db):
        """is_superuser=True bypasses policy."""
        user = MagicMock()
        user.is_superuser = True
        user.role = "admin"
        user.organization_id = 1
    ...
    def test_no_org_id_passthrough(self, mock_db):
        """User without org_id passes through."""
        user = MagicMock()
    ...
    def test_no_policy_passthrough(self, mock_db, regular_user):
        """No policy record → allowed."""
        mock_db.query.return_value.filter.return_value.first.return_value = None
        checker = check_module_write_policy("funds")
        req = self._make_request("POST")
        result = checker(request=req, current_user=regular_user, db=mock_db)
    ...
    def test_full_edit_passthrough(self, mock_db, regular_user):
        """Policy with full_edit → allowed."""
        policy = MagicMock()
        policy.edit_mode = "full_edit"
        mock_db.query.return_value.filter.return_value.first.return_value = policy
        checker = check_module_write_policy("funds")
        req = self._make_request("POST")
        result = checker(request=req, current_user=regular_user, db=mock_db)
    ...
    def test_read_only_blocked(self, mock_db, regular_user):
        """read_only policy blocks POST → 403."""
        policy = MagicMock()
        policy.edit_mode = "read_only"
        mock_db.query.return_value.filter.return_value.first.return_value = policy
        checker = check_module_write_policy("funds")
        req = self._make_request("POST")
        with pytest.raises(HTTPException) as exc_info:
            checker(request=req, current_user=regular_user, db=mock_db)
        assert exc_info.value.status_code == 403
        assert "只读" in exc_info.value.detail

    def test_disabled_blocked(self, mock_db, regular_user):
        """disabled policy blocks PUT → 403."""
        policy = MagicMock()
        policy.edit_mode = "disabled"
        mock_db.query.return_value.filter.return_value.first.return_value = policy
        checker = check_module_write_policy("funds")
    ...
    def test_delete_blocked(self, mock_db, regular_user):
        """disabled policy blocks DELETE → 403."""
        policy = MagicMock()
        policy.edit_mode = "disabled"
        mock_db.query.return_value.filter.return_value.first.return_value = policy
        checker = check_module_write_policy("map")
...
class TestCheckModuleReadPolicy:
    def test_superuser_bypass(self, mock_db):
        """super_admin bypasses read policy."""
        user = MagicMock()
        user.is_superuser = True
        user.role = "super_admin"
        user.organization_id = 1
        checker = check_module_read_policy("funds")
        result = checker(current_user=user, db=mock_db)
        assert result is None
    ...
    def test_no_policy_passthrough(self, mock_db, regular_user):
        ...
        mock_db.query.return_value.filter.return_value.first.return_value = None
        checker = check_module_read_policy("funds")
        result = checker(current_user=regular_user, db=mock_db)
        assert result is None

    def test_read_only_allowed(self, mock_db, regular_user):
        """read_only policy allows reads."""
        policy = MagicMock()
        policy.edit_mode = "read_only"
        mock_db.query.return_value.filter.return_value.first.return_value = policy
        checker = check_module_read_policy("funds")
    ...
    def test_full_edit_allowed(self, mock_db, regular_user):
        """full_edit policy allows reads."""
        policy = MagicMock()
        policy.edit_mode = "full_edit"
        mock_db.query.return_value.filter.return_value.first.return_value = policy
        checker = check_module_read_policy("funds")
    ...
    def test_disabled_blocked(self, mock_db, regular_user):
        """disabled policy blocks reads → 403."""
        policy = MagicMock()
        policy.edit_mode = "disabled"
        mock_db.query.return_value.filter.return_value.first.return_value = policy
        checker = check_module_read_policy("funds")
        with pytest.raises(HTTPException) as exc_info:
            checker(current_user=regular_user, db=mock_db)
        assert exc_info.value.status_code == 403
        assert "禁用" in exc_info.value.detail

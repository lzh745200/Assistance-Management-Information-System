"""Tests for app.api.v1.villages — using fixed Village model."""
...
class TestListVillages:
    def test_empty(self, client, mock_db):
        with patch("app.api.v1.villages.selectinload", return_value="mock"), \
             patch("app.core.data_permission.filter_by_data_scope", return_value=mock_db):
    ...
    def test_none_relations(self, client, mock_db):
        with patch("app.api.v1.villages.selectinload", return_value="mock"), \
             patch("app.core.data_permission.filter_by_data_scope", return_value=mock_db):
            v = make_mock_village()

; 帮扶管理信息系统 - NSIS x64 安装脚本
; 生成 64-bit 单文件 .exe 安装程序

!define PRODUCT_NAME "帮扶管理信息系统"
!define PRODUCT_VERSION "1.2.0"
!define PRODUCT_PUBLISHER "帮扶管理信息系统"
!define PRODUCT_DIR_REGKEY "Software\Microsoft\Windows\CurrentVersion\App Paths\assistance-system-x64.exe"
...
OutFile "..\dist\windows\x64\帮扶管理信息系统_${PRODUCT_VERSION}_win_x64.exe"
...
Function .onInit
  ${IfNot} ${RunningX64}
    MessageBox MB_OK|MB_ICONSTOP "此安装包仅支持 64-bit Windows 系统。请使用 32-bit 安装包。"
    Abort
  ${EndIf}
FunctionEnd
...
Section "MainSection" SEC01
  ...
  File "..\dist\windows\x64\backend\assistance-management-backend.exe"

  ; 复制前端文件
  SetOutPath "$INSTDIR\frontend"
  File /r "..\resources\frontend\*.*"

  ; 复制 VC++ Redistributable
  SetOutPath "$INSTDIR\redist"
  File /nonfatal "..\resources\vcredist\vc_redist.x64.exe"
  ...
  FileWrite $0 "echo 正在启动 帮扶管理信息系统...$\r$\n"
  FileWrite $0 "echo.$\r$\n"
  ...
  IfFileExists "$INSTDIR\redist\vc_redist.x64.exe" 0 skip_vcredist
...
Section -Post
  ...
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\${PRODUCT_NAME}" "DisplayName" "${PRODUCT_NAME} (64-bit)"

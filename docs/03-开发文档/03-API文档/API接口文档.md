...
### 2026-08-01
- ✅ 文件上传端点 (`POST /files/upload`) 响应格式统一为信封格式 `{code:200, data:{url, filename, size}, message:"成功"}`
- ✅ 角色体系精简为 4 个核心角色：`super_admin`/`admin`/`user`/`viewer`
- ✅ 新增 `normalize_role()` 角色归一化函数，旧角色自动映射

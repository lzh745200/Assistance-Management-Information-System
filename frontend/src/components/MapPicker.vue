<template>
  <div class="map-picker">
    <div class="picker-inputs">
      <el-row :gutter="12">
        <el-col :span="10">
          <el-input v-model.number="innerLng" placeholder="经度 (如 107.52)" :disabled="disabled" @change="onInputChange">
            <template #prepend>经度</template>
          </el-input>
        </el-col>
        <el-col :span="10">
          <el-input v-model.number="innerLat" placeholder="纬度 (如 26.26)" :disabled="disabled" @change="onInputChange">
    ...
    <el-dialog v-model="dialogVisible" title="在地图上选取坐标 — 点击地图即可选点" width="850px" destroy-on-close @opened="onDialogOpened">
      <OfflineMap
        ref="pickerMapRef"
        height="480px"
        :markers="mapMarkers"
      ...
      <div v-if="pickedLng != null" class="picked-info">
        <el-tag type="warning" size="large">已选坐标: {{ pickedLng.toFixed(6) }}, {{ pickedLat.toFixed(6) }}</el-tag>
      </div>
      <template #footer>
        <el-button @click="dialogVisible = false">取消</el-button>
        <el-button type="primary" :disabled="pickedLng == null" @click="confirmPick">确认选择</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
const mapMarkers = computed(() => {
  if (pickedLng.value == null || pickedLat.value == null) {
    ...
    if (innerLng.value && innerLat.value) {
      return [{ lng: innerLng.value, lat: innerLat.value, name: '当前位置', type: 'default' }]
    }
    return []
  }
  return [{ lng: pickedLng.value, lat: pickedLat.value, name: '已选位置', type: 'default' }]
})
...
.picker-inputs { margin-bottom: 8px; }
.picked-info { text-align: center; margin-top: 10px; }
</style>

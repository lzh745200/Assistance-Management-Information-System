...
function initChart() {
  ...
  chart.on('click', 'series', (params: any) => {
    if (params.seriesType === 'scatter' || params.seriesType === 'effectScatter') {
      ...
      emit('region-click', { name: params.name, lng: cp?.[0] || null, lat: cp?.[1] || null })
    }
  })
...
function renderMap() {
  ...
  features.forEach((f: any) => {
    ...
    if (pref && !prefSet.has(pref)) { prefSet.add(pref); prefList.push(pref) }
  ...
  const option: echarts.EChartsOption = {
    ...
    geo: {
      ...
      emphasis: {
        ...
        itemStyle: { areaColor: '#ffe082', borderColor: '#e65100', borderWidth: 2 },
      },
      itemStyle: {
        borderColor: '#999',
        borderWidth: 0.5,
    ...
    series: [
      // 县市区层 — 显示数据统计
      {
        ...
        type: 'map',
        geoIndex: 0,
        data: props.regionData.length > 0
          ? props.regionData
          : mapData,
        label: { show: false },
      },

...
class LocalDatabaseClass {
   ...
   * 查询数据（支持简单过滤）
   */
  async query<T = any>(tableName: string, filter?: Record<string, any>): Promise<T[]> {
   ...
   * 根据ID获取单条数据
   */
  async get<T = any>(tableName: string, id: string | number): Promise<T | null> {
    await this.init()
    ...
    if (this.db) {
      return new Promise((resolve) => {
        try {
          ...
          request.onerror = () => resolve(this.getByIdFromLocalStorage(tableName, id))
        } catch {
    ...
    if (this.db) {
      return new Promise((resolve) => {
        try {
          ...
          request.onerror = () => resolve(this.addToLocalStorage(tableName, data))
        } catch {
    ...
    if (this.db) {
      return new Promise((resolve) => {
        try {
          ...
          request.onerror = () => resolve(this.updateInLocalStorage(tableName, data))
        } catch {
    ...
    if (this.db) {
      return new Promise((resolve) => {
        try {
          ...
          request.onerror = () => resolve(this.deleteFromLocalStorage(tableName, id))
        } catch {
  ...
  async checkStorage(): Promise<{
    ...
    try {
      ...
      logger.warn('无法获取存储信息', error instanceof Error ? error : new Error(String(error)))
    }
  ...
  private getByIdFromLocalStorage<T = any>(tableName: string, id: string | number): T | null {
  ...
  private addToLocalStorage(tableName: string, data: any): any {
    const items = this.getFromLocalStorage(tableName)
    const newId = items.length > 0 ? Math.max(...items.map((i: any) => i.id || 0)) + 1 : 1
    const newItem = { ...data, id: newId }
    items.push(newItem)
    localStorage.setItem(`${this.storageKey}_${tableName}`, JSON.stringify(items))
    return newItem
  }
  ...
  private deleteFromLocalStorage(tableName: string, id: number | string): boolean {
    const items = this.getFromLocalStorage(tableName)
    const filtered = items.filter((i: any) => i.id !== id && String(i.id) !== String(id))
    localStorage.setItem(`${this.storageKey}_${tableName}`, JSON.stringify(filtered))
    return true
  }
}

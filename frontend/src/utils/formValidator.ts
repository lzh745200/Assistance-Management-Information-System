...
export class FormValidator {
  ...
  private debouncedValidators: Map<string, ReturnType<typeof createDebounce>> = new Map()
  ...
  validateFieldDebounced(
    ...
    if (!debounced) {
      debounced = createDebounce(async (val: any, cb?: (result: ValidationResult) => void) => {
 ...
 * @param options 配置选项
 */
export function createFormValidator(options?: FormValidatorOptions): FormValidator {
...
/** 长度范围规则 */
export const lengthRule = (min: number, max: number, message?: string): ValidationRule => ({
  min,
  max,

...
class Logger {
  ...
  private _normalizeContext(context?: unknown): Record<string, unknown> | undefined {
 ...
 * 包装同步函数，捕获异常并记录日志
 */
export function tryCatch<T>(fn: () => T, errorMessage?: string, defaultValue?: T): T | undefined {
  try {
    return fn()
  } catch (error) {
    logger.error(

...
export function getMockResponse(method: string, url: string): any | null {
  ...
  if (m === 'GET') {
    ...
    if (path === '/villages' || path.startsWith('/villages/')) {
      ...
      if (id !== null) {
        return detailResponse(mockVillages.find((v) => v.id === id) || mockVillages[0])
      }
    ...
    if (path === '/schools' || path.startsWith('/schools')) {
      ...
      if (id !== null) {
        return detailResponse(mockSchools.find((s) => s.id === id) || mockSchools[0])
      }
    ...
    if (path === '/projects' || path.startsWith('/projects')) {
      ...
      if (id !== null) {
        return detailResponse(mockProjects.find((p) => p.id === id) || mockProjects[0])
      }
    ...
    if (path === '/policies' || path.startsWith('/policies')) {
      ...
      if (id !== null) {
        return detailResponse(mockPolicies.find((p) => p.id === id) || mockPolicies[0])
      }
    ...
    if (path.startsWith('/approval')) {
      ...
      if (workflowId !== null) {
        return {
          data: mockApprovalWorkflows.find((w) => w.id === workflowId) || mockApprovalWorkflows[0],
        }
      }
    ...
    if (path.startsWith('/work-logs')) {
      ...
      if (id !== null) {
        return detailResponse(mockWorkLogs.find((w) => w.id === id) || mockWorkLogs[0])
      }
  ...
  if (m === 'POST') {
    ...
    return successResponse({ id: 'offline-' + Date.now() }, '操作成功（离线模式）')
  }

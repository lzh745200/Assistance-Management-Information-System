  ...
  return defineAsyncComponent({
    ...
    onError(error, retry, fail, attempts) {
      if (attempts <= mergedConfig.retryCount) {
        logger.warn(`[AsyncComponent] 加载错误，尝试重试 (${attempts}/${mergedConfig.retryCount})`)
        retry()
      } else {
 ...
 * 关键路由列表（需要优先预加载）
 */
export const CRITICAL_ROUTES = ['Dashboard', 'Projects', 'Villages', 'Statistics']
...
class PerformanceMonitorClass {
   ...
   * 生成性能报告
   */
  getReport(): Record<string, { count: number; average: string; max: string; min: string }> {
    const report: Record<string, { count: number; average: string; max: string; min: string }> = {}
 ...
 */
export function lazyLoad<T>(importFunc: () => Promise<T>, componentName: string): () => Promise<T> {
  return async () => {
    const start = performance.now()

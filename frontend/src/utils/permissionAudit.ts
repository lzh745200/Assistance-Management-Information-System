...
import { type Role, type Permission, rolePermissions, routePermissions } from '@/router/permissions'
    ...
    for (const route of routeList) {
      ...
      if (meta) {
        ...
        if (meta.requiresAdmin && Array.isArray(meta.roles)) {
          ...
          const hasNormalRole = roles.some((r) => NORMAL_ROLES.includes(r as Role))
          if (hasNormalRole) {
            issues.push({
              level: 'error',
              route: fullPath,
        ...
        if (Array.isArray(meta.requiredPermissions)) {
          for (const perm of meta.requiredPermissions as string[]) {
            const rolesWithPerm = (Object.keys(rolePermissions) as Role[]).filter((r) =>
              rolePermissions[r].includes(perm as Permission)
            )
...
export function runPermissionAudit(routes: RouteRecordRaw[]): void {
  ...
  if (allIssues.length === 0) {
    logger.info('%c✅ 权限审计通过，未发现冲突', 'color: green; font-weight: bold')
    return
  }

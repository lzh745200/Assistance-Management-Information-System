...
export class StateManager {
   ...
   * 获取状态
   */
  getState<T>(key: string, defaultValue?: T, options: StorageOptions = {}): T | undefined {
  ...
  clearState(prefix?: string): void {
    const fullPrefix = prefix ? this.getFullKey('', prefix) : this.storagePrefix
   ...
   * 订阅状态变化
   */
  subscribe(key: string, callback: Function, options: StorageOptions = {}): () => void {
    const { prefix = '' } = options
    const fullKey = this.getFullKey(key, prefix)
   ...
   * 批量设置状态
   */
  batchSetState(states: Record<string, any>, options: StorageOptions = {}): void {
   ...
   * 批量获取状态
   */
  batchGetState<T>(keys: string[], options: StorageOptions = {}): Record<string, T> {

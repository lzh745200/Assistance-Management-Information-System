...
async function fetchData() {
  try {
    const [projRes, fundRes] = await Promise.all([
      request.get("/projects", { params: { summary: true, page_size: 5 } } as any),
...
function buildBarOption(projects: any[]): echarts.EChartsCoreOption {
  ...
  return {
    ...
    xAxis: { type: "value", max: 100, axisLabel: { color: "#94a3b8", fontSize: 11 }, splitLine: { lineStyle: { color: "#f1f5f9", type: "dashed" } } },
    yAxis: { type: "category", data: names.reverse(), axisLine: { show: false }, axisTick: { show: false }, axisLabel: { color: "#475569", fontSize: 12 } },
    series: [{
      type: "bar", data: values.reverse(), barWidth: 16,
      itemStyle: {
        borderRadius: [0, 6, 6, 0],
        color: new echarts.graphic.LinearGradient(0, 0, 1, 0, [
          { offset: 0, color: "#2d6a4f" }, { offset: 1, color: "#52b788" },
        ]),
      },
...
function buildPieOption(funds: { allocated: number; pending: number; planned: number }): echarts.EChartsCoreOption {
  return {
...
function initCharts(data: { projects: any[]; funds: any }) {
  if (barRef.value) {
    ...
    barChart.setOption(buildBarOption(data.projects.length ? data.projects : [
      { name: "道路硬化", progress: 85 }, { name: "饮水工程", progress: 60 }, { name: "电商中心", progress: 40 },
    ]));
  }
  if (pieRef.value) {
...
.chart-title {
  font-size: 15px; font-weight: 600; color: #1e293b; margin: 0 0 12px 0;

...
const handleViewDetail = async (version: VersionItem) => {
  try {
    const response = await request.get(
      `/data-packages/${packageId.value}/versions/${version.id}`,
    );
...
const doCompare = async () => {
  ...
  try {
    const response = await request.get(
      `/data-packages/${packageId.value}/versions/compare`,
      {
        params: {
...
const handleDelete = async (versionId: number | string) => {
  try {
    ...
    const response = await request.delete(
      `/data-packages/${packageId.value}/versions/${versionId}`,
    );

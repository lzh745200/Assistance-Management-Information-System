          ...
          <el-table v-loading="loadingList" :data="packageList" border stripe>
            ...
            >
              <template #default="{ row }">
                <el-tag
                  :type="row.type === 'task' ? 'warning' : 'success'"
                  size="small"
                >
...
async function createTaskPackage() {
  ...
  try {
    ...
    try {
      const userStr =
        JSON.stringify(AuthStorage.getUser()) ||
        JSON.stringify(AuthStorage.getUser());
      if (userStr) {
    ...
    const errorMsg =
      error?.response?.data?.detail || error?.message || "生成任务包失败";

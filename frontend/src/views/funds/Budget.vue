  ...
  <div class="budget-page">
    ...
    <div class="page-header">
      <div class="header-left">
        <el-button :icon="ArrowLeft" @click="pushSafe('/funds')"
          >返回经费管理</el-button
        >
        ...
        >
          ...
          <el-table-column label="使用率" width="200">
            <template #default="{ row }">
              <el-progress
                :percentage="
                  row.budget > 0
                    ? Math.min(Math.round((row.used / row.budget) * 100), 100)
                    : 0
                "
                :color="
                  getProgressColor(
                    row.budget > 0 ? (row.used / row.budget) * 100 : 0,
                  )
                "
                :stroke-width="10"
              />
          ...
          <el-table-column label="操作" width="150" fixed="right">
            <template #default="{ row }">
              ...
              <el-popconfirm
                title="确定删除？"
                @confirm="handleDeleteBudget(row)"
              >
                <template #reference>
    ...
    >
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="saving" @click="handleSaveBudget"
          >保存</el-button
        >
      </template>
    </el-dialog>
...
const summary = computed(() => {
  const totalBudget = budgetData.value.reduce(
...
function getSummary({
  ...
  columns.forEach((_col, index: number) => {
    ...
    if (index === 1) {
      sums[index] = data
        .reduce((s: number, r) => s + (r.budget || 0), 0)
        .toFixed(2);
      return;
    }
    if (index === 2) {
      sums[index] = data
        .reduce((s: number, r) => s + (r.used || 0), 0)
        .toFixed(2);
      return;
    }
    if (index === 3) {

<template>
  <div class="policy-detail">
    <el-card v-loading="loading">
      <template #header><span>{{ policy?.title || '政策详情' }}</span></template>
      <el-descriptions v-if="policy" :column="2" border>
        ...
        <el-descriptions-item label="级别">{{ policy.level }}</el-descriptions-item>
...
onMounted(async () => {
  ...
  if (id) { loading.value = true; try { /* fetch */ } finally { loading.value = false; } }
});
</script>

<template>
  <div class="report-list-page">
    ...
    <el-card>
      <el-table v-loading="loading" :data="subscriptions" stripe border>
        ...
        <el-table-column prop="format" label="格式" width="80">
          <template #default="{ row }"
            ><el-tag size="small">{{ row.format || "xlsx" }}</el-tag></template
          >
        </el-table-column>
        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template #default="{ row }">
            <el-button size="small" @click="handleGenerate(row)"
              >生成</el-button
            >
      ...
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="输出格式" prop="format">
          <el-radio-group v-model="form.format"
            ...
            ><el-radio value="pdf">PDF</el-radio></el-radio-group
          >
        </el-form-item>
...
const typeOptions = [
  ...
  { label: "帮扶村报表", value: "supported_villages" },
  { label: "项目报表", value: "projects" },
  { label: "经费报表", value: "funds" },
  { label: "学校报表", value: "schools" },
  { label: "乡村振兴", value: "rural_work" },
];
...
async function handleGenerate(row: any) {
  try {
    await reportApi.generate({ subscription_id: row.id });

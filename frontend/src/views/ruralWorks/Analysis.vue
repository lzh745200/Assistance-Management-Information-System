<template>
  <div class="rural-works-analysis">
    ...
    <div class="page-header military-border">
      ...
      <p class="page-subtitle military-subtitle">
        深度分析乡村振兴工作数据，为决策提供科学依据
      </p>
      ...
      >
        ...
        <el-table-column prop="qualityScore" label="质量评分" width="100">
          <template #default="scope">
            <div
              class="quality-score"
              :class="getQualityScoreClass(scope.row.qualityScore)"
            >
...
const averageCompletionRate = computed(() => {
  ...
  const sum = completedOrInProgress.reduce(
    (acc, item) => acc + item.completionRate,
    0,
  );
...
const averageDelayRate = computed(() => {
  ...
  const delayedCount = filteredData.value.filter(
    (item) => item.status === "已延期",
  ).length;
...
const loadData = async () => {
  ...
  try {
    ...
    const villageSet = new Set(
      items.map((i: any) => i.village_name).filter(Boolean),
    );

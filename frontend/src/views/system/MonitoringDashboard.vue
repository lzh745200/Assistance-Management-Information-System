<template>
  <div class="monitoring-dashboard">
    ...
    <div class="metric-grid">
      ...
      >
        <el-popover placement="bottom" :width="200" trigger="hover" :show-after="300">
          <template #reference>
            <div class="card-inner">
              <div class="card-icon" :style="{ color: card.iconColor }">{{ card.icon }}</div>
              <div class="card-body">
                ...
                <el-tag :type="card.tagType" size="small">{{ card.statusText }}</el-tag>
              </div>
          ...
          <div class="popover-content">
            <div class="sparkline-bars">
              <div
                ...
                :key="i"
                class="spark-bar"
                :style="{ height: Math.max(4, pt * 100) + '%', backgroundColor: card.sparkColor }"
              />
            </div>
    ...
    <div class="middle-row">
      ...
      <div class="log-panel">
        ...
        <div class="log-container">
          ...
          <el-empty v-if="recentLogs.length === 0" description="暂无日志" :image-size="32" />
        </div>
      </div>
    </div>
    ...
    <div class="health-section">
      <div class="health-header" @click="healthExpanded = !healthExpanded">
        ...
        <span>系统健康检查</span>
        <el-tag :type="healthScore >= 80 ? 'success' : healthScore >= 60 ? 'warning' : 'danger'" size="small" style="margin-left:12px">
          {{ healthScore >= 90 ? '极佳' : healthScore >= 80 ? '良好' : healthScore >= 60 ? '需关注' : '异常' }}
        </el-tag>
      </div>
      <div v-show="healthExpanded" class="health-body">
        <div class="check-group">
          ...
          <div v-for="item in basicChecks" :key="item.name" class="check-item">
            <el-icon :size="16"><component :is="item.passed ? CircleCheckFilled : CircleCloseFilled" /></el-icon>
        ...
        <div class="check-group">
          ...
          <div class="check-item">
            ...
            <el-tag :type="dbInfo.integrityOk ? 'success' : 'danger'" size="small">{{ dbInfo.integrityOk ? '通过' : '失败' }}</el-tag>
          </div>
          <div class="check-item">
...
const healthExpanded = ref(sessionStorage.getItem("monitor-health-expanded") === "true");
...
const basicChecks = computed<CheckItem[]>(() => {
  const h = healthData.value;
  return [
    { name: "数据库连接", passed: h?.db_integrity_ok ?? false, detail: h?.db_integrity_ok ? "正常" : "异常" },
    { name: "API 服务", passed: apiStats.value.length > 0, detail: apiStats.value.length > 0 ? "正常响应" : "无数据" },
...
const performanceChecks = computed<CheckItem[]>(() => {
  ...
  return [
    { name: "CPU 使用率", passed: (s?.cpu_usage ?? 100) < 90, detail: s ? `${s.cpu_usage.toFixed(1)}%` : "--" },
    { name: "内存使用率", passed: (s?.memory_usage ?? 100) < 90, detail: s ? `${s.memory_usage.toFixed(1)}%` : "--" },
    { name: "磁盘使用率", passed: (s?.disk_usage ?? 100) < 90, detail: s ? `${s.disk_usage.toFixed(1)}%` : "--" },
    { name: "响应时间", passed: true, detail: "< 100ms" },
    { name: "数据库大小", passed: (healthData.value?.db_size_mb ?? 9999) < 500, detail: dbInfo.value.size },
  ];
});
...
function statusInfo(val: number, invert = false): { status: string; tagType: "success" | "warning" | "danger"; statusText: string } {
  const v = invert ? 100 - val : val;
  if (v >= 90) return { status: "danger", tagType: "danger" as const, statusText: "严重" };
  if (v >= 70) return { status: "warning", tagType: "warning" as const, statusText: "警告" };
...
const metricCards = computed<MetricCard[]>(() => {
  ...
  const thrSi: { status: string; tagType: "success" | "warning"; statusText: string } = s?.process_threads != null && s.process_threads > 500
  ...
  return [
    {
      key: "cpu", icon: "🖥️", iconColor: "#409eff",
      value: hasData ? s!.cpu_usage.toFixed(1) : "--",
      unit: "%", ...cpuSi, label: "CPU 使用率",
      detail: hasData ? `${s!.cpu_count} 核` : "",
      subDetail: hasData ? `进程线程: ${s!.process_threads}` : "",
      history: cpuHist, sparkColor: "#409eff", error: !hasData,
    },
    {
      key: "memory", icon: "💾", iconColor: "#67c23a",
      value: hasData ? s!.memory_usage.toFixed(1) : "--",
      unit: "%", ...memSi, label: "内存使用率",
      detail: hasData ? `${s!.memory_used_mb.toFixed(0)} / ${s!.memory_total_mb.toFixed(0)} MB` : "",
      subDetail: "", history: memHist, sparkColor: "#67c23a", error: !hasData,
    },
    {
      key: "disk", icon: "📀", iconColor: "#e6a23c",
      value: hasData ? s!.disk_usage.toFixed(1) : "--",
      unit: "%", ...diskSi, label: "磁盘使用率",
      detail: hasData ? `${s!.disk_used_gb.toFixed(1)} / ${s!.disk_total_gb.toFixed(1)} GB` : "",
      subDetail: "", history: diskHist, sparkColor: "#e6a23c", error: !hasData,
    },
    {
      key: "net_recv", icon: "📥", iconColor: "#909399",
      value: hasData ? s!.network_recv_mb.toFixed(1) : "--",
      unit: "MB", ...netRecvSi, label: "网络接收",
      detail: hasData ? "累计接收" : "", subDetail: "",
      history: [], sparkColor: "#909399", error: !hasData,
    },
    {
      key: "net_sent", icon: "📤", iconColor: "#909399",
      value: hasData ? s!.network_sent_mb.toFixed(1) : "--",
      unit: "MB", ...netSentSi, label: "网络发送",
      detail: hasData ? "累计发送" : "", subDetail: "",
      history: [], sparkColor: "#909399", error: !hasData,
    },
    {
      key: "threads", icon: "⚙️", iconColor: "#409eff",
      value: hasData ? String(s!.process_threads) : "--",
      unit: "", ...thrSi, label: "进程线程",
      detail: hasData ? `${s!.cpu_count} 核 CPU` : "",
      subDetail: "", history: [], sparkColor: "#409eff", error: !hasData,
    },
  ];
});
...
async function refreshAll() {
  ...
  try {
    ...
    if (snapData) {
      ...
      if (snapData.cpu_usage > 80) logs.push({ id: sid, time: new Date().toLocaleTimeString(), level: "warn", message: `CPU 使用率偏高: ${snapData.cpu_usage.toFixed(1)}%` });
      if (snapData.memory_usage > 80) logs.push({ id: sid + 1, time: new Date().toLocaleTimeString(), level: "warn", message: `内存使用率偏高: ${snapData.memory_usage.toFixed(1)}%` });
      if (snapData.disk_usage > 85) logs.push({ id: sid + 2, time: new Date().toLocaleTimeString(), level: "error", message: `磁盘使用率过高: ${snapData.disk_usage.toFixed(1)}%` });
      if (logs.length === 0) logs.push({ id: sid, time: new Date().toLocaleTimeString(), level: "info", message: "系统资源使用正常" });
...
function buildChart() {
  ...
  chartInstance.setOption({
    ...
    yAxis: [
      { type: "value", name: "请求数/耗时", splitLine: { lineStyle: { type: "dashed" } } },
...
.last-updated { font-size: var(--font-size-xs); color: var(--color-text-secondary); }
...
.health-score-num { font-size: 18px; font-weight: 700; line-height: 1; }
.health-score-label { font-size: 10px; color: var(--color-text-secondary); }
...
.metric-card.status-normal { border-left-color: #67c23a; }
.metric-card.status-warning { border-left-color: #e6a23c; }
.metric-card.status-danger { border-left-color: #f56c6c; }
...
.card-body { display: flex; align-items: baseline; justify-content: space-between; gap: 6px; }
.card-value { font-size: 28px; font-weight: 700; color: var(--color-text-primary); line-height: 1.2; }
.card-unit { font-size: 14px; font-weight: 400; color: var(--color-text-secondary); margin-left: 2px; }
.card-footer {
...
.log-container { max-height: 280px; overflow-y: auto; padding: 0 16px 8px; }
...
.log-level { font-weight: 600; flex-shrink: 0; width: 40px; }
.log-info .log-level { color: #409eff; }
.log-warn .log-level { color: #e6a23c; }
.log-error .log-level { color: #f56c6c; }
.log-message { color: var(--color-text-regular); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
.health-header:hover { background: var(--color-bg-hover); }
.toggle-icon { font-size: 12px; color: var(--color-text-secondary); }
...
.check-item:nth-child(odd) { background: var(--color-bg-hover); }
.check-item .el-icon { flex-shrink: 0; }
.check-item .el-icon :deep(svg) { color: #67c23a; }
.check-item:has(.el-icon + :deep(svg[style*="f56c6c"])) .el-icon :deep(svg),
.check-item .el-icon:has(+ .check-detail:empty) { color: #f56c6c; }
.check-name { flex-shrink: 0; min-width: 90px; font-weight: 500; color: var(--color-text-primary); }
.check-detail { color: var(--color-text-secondary); }
</style>

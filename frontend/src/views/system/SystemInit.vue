  ...
  <div class="system-init-page">
    <el-card class="init-card">
      ...
      <div v-else>
        ...
        <!-- Step 0: 单位信息 -->
        <el-form v-if="currentStep === 0" ref="orgFormRef" :model="form" :rules="orgRules" label-width="120px" class="init-form">
        ...
        <!-- Step 1: 管理员 -->
        <el-form v-if="currentStep === 1" ref="adminFormRef" :model="form" :rules="adminRules" label-width="120px" class="init-form">
        ...
        <div v-if="currentStep === 2" class="init-form">
          <el-descriptions title="确认配置信息" :column="2" border>
            <el-descriptions-item label="单位名称">{{ form.organization_name }}</el-descriptions-item>
          ...
          <div class="step-actions">
            ...
            <el-button type="primary" :loading="submitting" @click="executeInit">确认初始化</el-button>
          </div>
...
const adminRules = {
  ...
  confirmPassword: [
    ...
    { validator: (_r: any, v: string, cb: any) => cb(v !== form.value.admin_password ? new Error("两次密码不一致") : undefined), trigger: "blur" },
  ],
};
...
function nextStep(formRef: any) {
  if (!formRef) { currentStep.value++; return; }
  formRef.validate((valid: boolean) => { if (valid) currentStep.value++; });
}
...
function goToLogin() { pushSafe("/login"); }
</script>

<style scoped>
.system-init-page {
  display: flex; justify-content: center; align-items: center; min-height: 80vh;
  background: #081c15; padding: 20px;
}
.init-card { width: 100%; max-width: 680px; }
.card-header { text-align: center; }
.card-header h2 { color: #1b4332; }
.loading-box { text-align: center; padding: 60px; }
.init-form { margin-top: 30px; }
.step-actions { margin-top: 24px; display: flex; gap: 12px; justify-content: center; }
.error-msg { margin-top: 16px; color: #f56c6c; text-align: center; }
</style>

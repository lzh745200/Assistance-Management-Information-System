...
OutFile "..\dist\windows\${PRODUCT_NAME}_${PRODUCT_VERSION}_win_x86.exe"
...
Section "Install"
  ...
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} (32-bit).lnk" "$INSTDIR\assistance-management-backend.exe"
  ...
  WriteRegStr HKLM "Software\${PRODUCT_NAME_EN}" "Arch" "x86"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\${PRODUCT_NAME_EN}" "DisplayName" "${PRODUCT_NAME} (32-bit)"
